...
class AsyncSubscriptionsResource(AsyncBaseResource):
    async def list(self, organization_id: str) -> list[dict]:
        return await self._get(f"/organizations/{organization_id}/event-subscriptions", auth="bearer")
    ...
    async def activity(self, organization_id: str, subscription_id: str, *, limit: int = 10) -> dict:
